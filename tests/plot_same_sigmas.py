...
for st, rmax, lbl, c, zz in zip(structypes,r_maxs,ensemble_lbls,clrs,zorders):
    print(f'\n---------- {st} ----------')
    plot_sigmas = np.zeros((2,3))
    for x, mt in zip(range(1,4),motypes):
        if st == '40x40':
            dat1 = np.load(f'/Users/nico/Desktop/simulation_outputs/percolation/sigmas_v_T/sigma_v_T_w_err_rerun2_rmax_{rmax}_{mt}.npy').T
        else:
            dat1 = np.load(f'/Users/nico/Desktop/simulation_outputs/percolation/sigmas_v_T/sigma_v_T_w_err_{st}_rerun_rmax_{rmax}_{mt}.npy').T
        dat2 = np.load(f'/Users/nico/Desktop/simulation_outputs/percolation/sigmas_v_T/sandbox_run/{lbl}/sigma_v_T_w_err_production_{mt}.npy').T
        ...
        for k, dat in enumerate(dats):
            temps, sigmas, sig_errs = dat
            kselect = (temps==T).nonzero()[0]
            sigma = sigmas[kselect] 
            if k == 0:
                sigma *= (conv_factor / (kB*T))
            plot_sigmas[k,x-1] = sigma
            sig_err = sig_errs[kselect]
            if k == 0:
                sig_err = sig_err  * conv_factor / (kB*T)
            print(f'{mt} ---> {sigma} ± {sig_err}')
    
    ax.plot(range(1,4), plot_sigmas[0,:], f'{symbols[0]}-', ms=20.0, c=c,label='dat1',lw=1.5,alpha=0.6)
    ax.plot(range(1,4), plot_sigmas[1,:], f'{symbols[1]}-', ms=20.0, c=c,label='dat2',lw=1.5,alpha=0.6)
